...
const features = [
  {
    ...
    description:
      "Hero、特徴紹介、料金表、FAQ など、15 種類のセクションをすぐに呼び出して差し替えるだけ。Tailwind のユーティリティで柔軟に調整できます。",
    icon: <IconSparkles />,
  },
  {
    title: "リアルタイムプレビュー",
    description:
      "App Router をベースに、下書きの状態でもプレビュー URL を即共有。ドラフトと公開の差分も視覚的に追えます。",
    icon: <IconGauge />,
  },
  {
    title: "アクセシビリティ最優先",
    description:
      "ARIA ランドマーク、フォーカスリング、コントラストを最初から設計。ダークモードやキーボード操作にも完全対応しています。",
    icon: <IconShield />,
  },
] as const;
...
const plans: Plan[] = [
  ...
  },
  {
    ...
    perks: ["A/B テストセクション", "CMS & フォーム自動連携", "カスタムコンポーネント相談 (月2回)"],
  },
  {
    name: "Enterprise",
    price: "お見積り",
...
export default function Home() {
  return (
    <main id="main-content" className="flex-1">
      {/* ==== Hero（装飾SVG一切ナシ／最小差分でStep4反映） ==== */}
      <section id="hero" aria-labelledby="hero-title" className="container mx-auto max-w-7xl px-6 py-24">
  <div className="mx-auto flex flex-col gap-12 lg:flex-row lg:items-center lg:justify-between">
    <div className="max-w-2xl space-y-8">
      ...
      <h1
        id="hero-title"
        className="bg-gradient-to-r from-neonPink via-neonCyan to-neonPurple bg-clip-text text-transparent text-4xl md:text-6xl font-bold tracking-tight"
      >
      ...
      <div className="mt-10 flex flex-wrap items-center gap-4" aria-label="主要アクション">
        <form action="/contact">
          <Button type="submit" variant="neon" size="lg">デモを依頼する</Button>
        </form>
        <Link
          href="/demo/button"
          className="inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold underline underline-offset-4 focus:outline-none focus-visible:ring-2 ring-neonCyan ring-offset-2"
        >
          UI コンポーネントを確認 <span aria-hidden>→</span>
        </Link>
      </div>
    ...
    {/* 右側のミニカード（そのままでOK。SVGは h-6 w-6 なので消えません） */}
      ...
      >
        <dl className="space-y-6">
          {faqs.map((faq) => (
            <div
              key={faq.question}
              className="rounded-2xl border border-black/5 bg-white p-6 dark:border-white/10 dark:bg-black/40"
            >
        ...
        <div className="mx-auto max-w-3xl rounded-3xl border border-brand-200 bg-brand-50/60 p-10 text-center shadow-lg dark:border-brand-100/30 dark:bg-brand-100/10">
          <div className="mx-auto flex max-w-2xl flex-col gap-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <Link
                href="#features"
                className="inline-flex items-center justify-center rounded-lg border border-brand-300 px-5 py-3 text-sm font-semibold transition hover:bg-brand-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ring-neonCyan"
              >
                機能一覧を確認
              </Link>
            </div>
