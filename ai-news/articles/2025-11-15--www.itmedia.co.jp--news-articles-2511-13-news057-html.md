...
tags:
  ...
  - summary
kind: summary
tldr: Google Cloudは、生成AIサービスGeminiにPDFやWord、Excelなどの多様なファイルを検索・活用可能にするフルマネージドなRAGシステム「File Search in Gemini API」の提供を開始しました。これにより、企業は複雑なRAGシステム構築の手間なくAPI経由で社内文書などをGeminiに読み込ませ、より関連性の高い情報を基にした正確な回答を生成させることが可能になります。
model: ""
---
# GeminiにPDFやWord、Excel、テキストファイルなどの検索機能を組み込める「File Search in Gemini API」提供開始

> [!abstract] TL;DR
> - Google Cloudは、生成AIサービスGeminiにPDFやWord、Excelなどの多様なファイルを検索・活用可能にするフルマネージドなRAGシステム「File Search in Gemini API」の提供を開始しました
> - これにより、企業は複雑なRAGシステム構築の手間なくAPI経由で社内文書などをGeminiに読み込ませ、より関連性の高い情報を基にした正確な回答を生成させることが可能になります
## TL;DR

Google Cloudは、生成AIサービスGeminiにPDFやWord、Excelなどの多様なファイルを検索・活用可能にするフルマネージドなRAGシステム「File Search in Gemini API」の提供を開始しました。これにより、企業は複雑なRAGシステム構築の手間なくAPI経由で社内文書などをGeminiに読み込ませ、より関連性の高い情報を基にした正確な回答を生成させることが可能になります。
